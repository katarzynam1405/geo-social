...
function onGeolocationSuccess(position) {
    ...
    API(position.coords.latitude, position.coords.longitude).then(parseJson).then(onShowMoreClickHandler).catch(e => alert(e, "warning", 100000));
}
...
function onShowMoreClickHandler(dane){
    console.log(init+=20);
    console.log(added+=20);
    let images = dane.photos.photo.slice(init, added);
